...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse> handleResourceNotFound(ResourceNotFoundException ex, HttpServletRequest request) {
        log.error("GlobalExceptionHandler : handleResourceNotFound : Resource not found - {}", ex.getMessage());
    ...
    public ResponseEntity<ApiResponse> handleBadRequest(BadRequestException ex, HttpServletRequest request) {
        log.error("GlobalExceptionHandler : handleBadRequest : Bad request - {}", ex.getMessage());
    ...
    public ResponseEntity<ApiResponse> handleUnauthorized(UnauthorizedException ex, HttpServletRequest request) {
        log.error("GlobalExceptionHandler : handleUnauthorized : Unauthorized access - {}", ex.getMessage());
    ...
    public ResponseEntity<ApiResponse> handleUnexpected(UnexpectedException ex, HttpServletRequest request) {
        log.error("GlobalExceptionHandler : handleUnexpected : Unexpected error - {}", ex.getMessage());
    ...
    public ResponseEntity<ApiResponse> handleIllegalArgument(IllegalArgumentException ex, HttpServletRequest request) {
        log.error("GlobalExceptionHandler : handleIllegalArgument : Illegal argument - {}", ex.getMessage());
        ApiResponse response = ApiResponse.error(ex.getMessage(), "ILLEGAL_ARGUMENT", request.getRequestURI());
        return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<ApiResponse> handleValidation(MethodArgumentNotValidException ex, HttpServletRequest request) {
        ...
        log.error("GlobalExceptionHandler : handleValidation : Validation error - {}", message);
    ...
    public ResponseEntity<ApiResponse> handleGlobalException(Exception ex, HttpServletRequest request) {
        log.error("GlobalExceptionHandler : handleGlobalException : Internal server error - {}", ex.getMessage(), ex);
